    ...
    def log_system_action(self, action):
        try:
            ...
            cursor = self.db.execute_query(
                "SELECT id FROM system_logs WHERE path = ?", (log_file,)
            )
...
class HR:
    ...
    def load_hr_employee_table(self):
        ...
        try:
            ...
            self.system_logs.log_system_action(
                f"Database error while loading HR employees: {e}"
            )
    ...
    def load_hr_attendance_logs_table(self):
        ...
        try:
            ...
            self.system_logs.log_system_action(
                f"Database error while loading HR attendance logs: {e}"
            )
...
class Home:
        ...
        try:
            ...
            if admin_result:
                ...
                if password == db_password:
                    ...
                    self.system_logs.log_system_action(
                        "An invalid Admin loggin attempt has been made."
                    )
                    return
            ...
            if employee_result:
                ...
                if employee_data["is_hr"]:
                    ...
                    if self.validate_attendance():
                        self.system_logs.log_system_action(
                            "A user is logged in as an employee"
                        )
                        self.goto_bio1()
            else:
                self.show_error(
                ...
                self.system_logs.log_system_action(
                    "An invalid loggin attempt has been made."
                )
        except sqlite3.Error as e:
    ...
    def validate_attendance(self):
        ...
        if attendance:
            ...
            if (
                ...
                self.show_error(
                    "Invalid Logout",
                    "You cannot log out less than 8 hours after logging in.",
                )
                return False
    ...
    def goto_result_prompt(self):
        ...
        if self.employee_data:
            ...
            self.db.execute_query(
                ...
                (
                    ...
                    current_date,
                    current_time.strftime("%H:%M:%S"),
                    self.employee_data.get("remarks", "in"),
                ),
            )
        ...
        threading.Timer(
            2.0, lambda: self.home_ui.main_page.setCurrentWidget(self.home_ui.home_page)
        ).start()
...
class ChangePassword:
    def __init__(self, db, admin_id):
        ...
        self.system_logs.log_system_action(
            "The change password prompt has been loaded."
        )
    ...
    def validate_and_change_password(self):
        ...
        try:
            ...
            self.system_logs.log_system_action(
                "There was an Database error during password change"
            )
...
class Admin:
    ...
    def load_backup_and_archive_settings(self):
        try:
            cursor = self.db.execute_query(
                "SELECT backup_config, automatic_backup_config, archive_config FROM system_setting WHERE id = 1"
            )
    ...
    def goto_employee_enroll(self):
        self.system_logs.log_system_action(
            "The employee enrollment page has been opened."
        )
    ...
    def display_employee_view(self, employee_data):
        self.system_logs.log_system_action(
            "Loading all data of an employee for viewing."
        )
    ...
    def display_hr_view(self, hr_data):
        self.system_logs.log_system_action(
            "Loading all data of an HR employee for viewing."
        )
    ...
    def save_edited_employee(self):
        ...
        try:
            ...
            self.system_logs.log_system_action(
                "A successful edit of an employee has been made."
            )
            success_msg = QMessageBox()
            success_msg.setIcon(QMessageBox.Information)
    ...
    def toggle_employee_status(self):
        ...
        try:
            ...
            self.db.execute_query(
                ...
                (new_status, employee["employee_id"]),
            )
    ...
    def validate_employee_data(self):
        ...
        employee_data = {
            ...
            "password": password,
            "gender": gender,
            "birthday": self.admin_ui.employee_birthday_edit.date().toString(
                "yyyy-MM-dd"
            ),
            "department": department,
            "position": position,
    ...
    def load_employee_table(self):
        self.system_logs.log_system_action(
            "Loading all employees to the employee table."
        )
        try:
            cursor = self.db.execute_query("SELECT * FROM Employee WHERE is_hr = 0")
            employees = cursor.fetchall() if cursor else []
    ...
    def save_backup_config(self):
        try:
            ...
            auto_backup_config = (
                1
                if self.admin_ui.auto_back_choice_1.isChecked()
                else 2 if self.admin_ui.auto_back_choice_2.isChecked() else 3
            )
    ...
    def save_archive_config(self):
        try:
            archive_config = (
                1
                if self.admin_ui.archive_choice_1.isChecked()
                else 2 if self.admin_ui.archive_choice_2.isChecked() else 3
            )
