...
class DatabaseConnection:
    def __init__(self, db_name="eals_database"):
        ...
        try:
            with self.lock:
                ...
                CREATE TABLE IF NOT EXISTS system_logs (
                    ...
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    last_modified_at TIMESTAMP NULL DEFAULT NULL,
                    entity_started VARCHAR(20),
                    stopped_to_the_entity VARCHAR(20),
                ...
                cursor.execute('''
                INSERT IGNORE INTO Admin (admin_id, password, password_changed) 
    ...
    def log_system_action(self, action, entity_type):
        ...
        try:
            ...
            cursor = self.db.execute_query(
                "SELECT id FROM system_logs WHERE DATE(created_at) = %s",
                (today,)
            )
            ...
            if existing_log:
                self.db.execute_query(
                    '''
                    UPDATE system_logs
                    SET last_modified_at = %s, 
                        stopped_to_the_entity = %s
                    WHERE id = %s
                    ''',
...
class HR:
    ...
    def update_hr_dashboard_labels(self):
        try:
            ...
            cursor = self.db.execute_query(
                ...
                "WHERE date = %s AND employee_id IN (SELECT employee_id FROM Employee WHERE is_hr = 0)", 
                (today_date,)
            )
            ...
            cursor = self.db.execute_query(
                "SELECT COUNT(DISTINCT employee_id) FROM attendance_logs "
                "WHERE date = %s AND remarks = 'late' AND employee_id IN (SELECT employee_id FROM Employee WHERE is_hr = 0)", 
                (today_date,)
            )
            ...
            cursor = self.db.execute_query(
                ...
                "SELECT DISTINCT employee_id FROM attendance_logs WHERE date = %s)", 
                (today_date,)
            )
    ...
    def handle_logout(self):
        ...
        self.db.execute_query("INSERT INTO attendance_logs (employee_id, date, time, remarks) VALUES (%s, %s, %s, %s)",
    ...
    def load_hr_employee_attendance_logs(self, employee_id):
        ...
        try:
            cursor = self.db.execute_query(
                "SELECT date, time, remarks FROM attendance_logs WHERE employee_id = %s", (employee_id,)
            )
            logs = cursor.fetchall() if cursor else []
...
class Home:
    ...
    def validate_hr_attendance(self, hr_data):
        ...
        cursor = self.db.execute_query("SELECT time, remarks FROM attendance_logs WHERE employee_id = %s AND date = %s", 
    ...
    def handle_login(self):
        ...
        try:
            cursor = self.db.execute_query("SELECT password, password_changed FROM Admin WHERE admin_id = %s", (user_id,))
            ...
            cursor = self.db.execute_query("SELECT * FROM Employee WHERE employee_id = %s AND password = %s", (user_id, password))
    ...
    def validate_attendance(self):
        ...
        cursor = self.db.execute_query(
            "SELECT time, remarks FROM attendance_logs WHERE employee_id = %s AND date = %s", 
    ...
    def goto_result_prompt(self):
        if self.employee_data:
            ...
            self.db.execute_query(
                "INSERT INTO attendance_logs (employee_id, date, time, remarks) VALUES (%s, %s, %s, %s)", 
            ...
            try:
                cursor = self.db.execute_query(
                    "SELECT remarks, date, time FROM attendance_logs WHERE employee_id = %s", 
...
class ChangePassword:
    def __init__(self, db, admin_id):
        ...
        self.system_logs.log_system_action("The change password prompt has been loaded.", "Admin")
    ...
    def validate_and_change_password(self):
        ...
        try:
            cursor = self.db.execute_query("SELECT password FROM Admin WHERE admin_id = %s", (self.admin_id,))
            ...
            self.db.execute_query("UPDATE Admin SET password = %s, password_changed = TRUE WHERE admin_id = %s", (new_password, self.admin_id))

            success_msg = QMessageBox()
            success_msg.setIcon(QMessageBox.Information)
            self.system_logs.log_system_action("The admin password has been changed.", "Admin")
            ...
            self.system_logs.log_system_action("There was an Database error during password change", "Admin")
...
class Admin:
    ...
    def validate_edited_employee_data(self):
        ...
        try:
            cursor = self.db.execute_query(
                "SELECT employee_id FROM Employee WHERE employee_id = %s AND employee_id != %s",
            ...
            cursor = self.db.execute_query(
                "SELECT first_name, last_name FROM Employee WHERE first_name = %s AND last_name = %s AND employee_id != %s",
    ...
    def save_edited_employee(self):
        ...
        try:
            ...
            self.db.execute_query('''
                UPDATE Employee
                SET first_name = %s, last_name = %s, middle_initial = %s, birthday = %s, gender = %s,
                    department = %s, position = %s, schedule = %s, is_hr = %s, status = %s, 
                    password = %s, profile_picture = %s, last_modified_by = %s, last_modified_at = %s
                WHERE employee_id = %s
            ''', (
    ...
    def toggle_employee_status(self):
        ...
        try:
            ...
            self.db.execute_query('''
                UPDATE Employee 
                SET status = %s, last_modified_by = %s, last_modified_at = %s 
                WHERE employee_id = %s
            ...
            self.system_logs.log_system_action(
                f"Employee {employee['employee_id']} status changed to {new_status} by {current_admin}", "Employee"
            )
    ...
    def validate_employee_data(self):
        ...
        employee_id = f"{id_pref}-{id_year}-{id_no}"
        try:
            cursor = self.db.execute_query("SELECT employee_id FROM Employee WHERE employee_id = %s", (employee_id,))
            if cursor.fetchone():
                return False, f"Employee ID {employee_id} already exists in the database."

            # Check if the name already exists
            cursor = self.db.execute_query(
                "SELECT first_name, last_name FROM Employee WHERE first_name = %s AND last_name = %s",
    ...
    def handle_delete_employee(self):
        ...
        if confirm_msg.exec() == QMessageBox.Yes:
            try:
                self.db.execute_query("DELETE FROM attendance_logs WHERE employee_id = %s", 
                                    (employee_data['employee_id'],))
                
                self.db.execute_query("DELETE FROM feedback WHERE created_by = %s", 
                                    (employee_data['employee_id'],))
                
                self.db.execute_query("DELETE FROM Employee WHERE employee_id = %s", 
                                    (employee_data['employee_id'],))
    ...
    def save_employee_data(self, employee_data):
        ...
        try:
            ...
            cursor = self.db.execute_query("SELECT employee_id FROM Employee WHERE employee_id = %s", 
                                        (employee_data['employee_id'],))
            result = cursor.fetchone() if cursor else None
    ...
    def load_hr_attendance_logs(self, hr_id):
        try:
            cursor = self.db.execute_query(
                "SELECT date, time, remarks FROM attendance_logs WHERE employee_id = %s", (hr_id,)
            )
            logs = cursor.fetchall() if cursor else []
        ...
        try:
            ...
            if config_exists:
                self.db.execute_query('''
                    ...
                    SET backup_type = %s, backup_frequency = %s, backup_unit = %s,
                        retention_enabled = %s, retention_frequency = %s, retention_unit = %s,
