...
class Home:
    ...
    def check_first_time_setup(self):
        ...
        cursor = self.db.execute_query("SELECT COUNT(*) FROM Admin")
        count = cursor.fetchone()[0] if cursor else 0
        
        if count == 0:
            # First time setup - show start1 page
            self.home_ui.main_page.setCurrentWidget(self.home_ui.start1_page)
        else:
            # Not first time - show login page
            self.home_ui.main_page.setCurrentWidget(self.home_ui.home_page)

    def goto_start2_page(self):
        # Generate admin credentials 
        admin_id, password = self.generate_admin_credentials()
        
        # Display credentials
        self.home_ui.admin_userid_lbl.setText(admin_id)
        self.home_ui.admin_pass_lbl.setText(password)
        
        # Hash password and store in database
        hashed_password = PASSWORD_HASHER.hash(password)
        self.db.execute_query(
            "INSERT INTO Admin (admin_id, password, password_changed) VALUES (?, ?, ?)",
            (admin_id, hashed_password, False)
        )
        
        # Switch to credentials display page
        self.home_ui.main_page.setCurrentWidget(self.home_ui.start2_page)
        
        # Set timer to auto-switch to login page after 10 seconds
        QTimer.singleShot(10000, lambda: self.home_ui.main_page.setCurrentWidget(self.home_ui.home_page))

    def generate_admin_credentials(self):
        # Generate random 8-character admin ID suffix
        suffix = ''.join(secrets.choice('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz') for _ in range(8))
        admin_id = f"admin-{suffix}"
        
        # Generate random 8-character password with mix of letters and numbers
        password = ''.join(secrets.choice('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz') for _ in range(8))
...
class ForgotPassword:
    ...
    def __init__(self, db):
        ...
        self.forgot_pass_ui.setWindowFlags(Qt.Window | Qt.WindowTitleHint | Qt.CustomizeWindowHint | Qt.WindowStaysOnTopHint)
