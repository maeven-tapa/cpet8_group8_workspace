...
import time
import cv2
from splash import EALS_SplashScreen
...
def move_ui_and_resources_from_internal():
    # This function moves 'ui' and 'resources' from _internal to the main dist folder if needed
    base_dir = os.path.dirname(os.path.abspath(sys.argv[0]))
    internal_dir = os.path.join(base_dir, "_internal")
    for folder in ["ui", "resources"]:
        src = os.path.join(internal_dir, folder)
        dst = os.path.join(base_dir, folder)
        if os.path.exists(src) and not os.path.exists(dst):
            try:
                shutil.move(src, dst)
                print(f"Moved {folder} from _internal to {base_dir}")
...
class FingerprintLogic:
    ...
    def display_image(self, image_data, fp_image_lbl):
        ...
        img = PILImage.frombytes('L', (width, height), image_data)
    ...
    def _register_fingerprint_temp_worker(self, employee_id, fp_image_lbl, fp_enrollment_note_lbl, completion_callback):
        ...
        try:
            ...
            if self.is_fingerprint_already_used(reg_temp, employee_id):
                fp_enrollment_note_lbl.setText("This fingerprint matches one already in the system. Re-enrollment failed.")
                self.enrollment_in_progress = False
                completion_callback(False)
                return
...
class EALS:
    def __init__(self):
        # Don't initialize everything here anymore
        pass
...
def show_splash_and_initialize():
    """Show splash screen and initialize the application"""
    global _initial_startup
    
    if not _initial_startup:
        # If not initial startup (e.g., from logout), skip splash
        controller = EALS()
        home_ui = controller.initialize_application()
        home_ui.showMaximized()
        return controller
    ...
    try:
        ...
        splash.findChild(QLabel).setText("Loading database...")
        app.processEvents()
        time.sleep(0.5)
        
        home_ui = controller.initialize_application()

        splash.findChild(QLabel).setText("Loading interface...")
        app.processEvents()
        time.sleep(0.5)
...
class HR:
    ...
    def setup_avg_work_hours_line_chart(self):
        ...
        self.avg_chart.setBackgroundBrush(QColor(250, 250, 250))
    ...
    def setup_attendance_pie_chart(self):
        ...
        self.pie_chart.setBackgroundBrush(QColor(250, 250, 250))
    ...
    def setup_top_present_bar_chart(self):
        ...
        self.top_present_chart.setAnimationOptions(QChart.SeriesAnimations)
        ...
        self.top_present_chart.setBackgroundBrush(QColor(250, 250, 250))
    ...
    def setup_top_late_bar_chart(self):
        ...
        self.top_late_chart.setAnimationOptions(QChart.SeriesAnimations)
        self.top_late_set.setColor(late_color)
        self.top_late_chart.addSeries(self.top_late_series)
        self.top_late_chart.setBackgroundBrush(QColor(250, 250, 250))
    ...
    def handle_logout(self):
        global _initial_startup
        _initial_startup = False  # Ensure splash doesn't show
...
class Admin:
    def __init__(self, db):
        ...
        self.setup_table_selection_handlers()
    ...
    def setup_table_selection_handlers(self):
        """Ensure only one table has a selection at a time."""
        self.admin_ui.employee_list_tbl.itemSelectionChanged.connect(self.handle_employee_table_selection)
        self.admin_ui.hr_list_tbl.itemSelectionChanged.connect(self.handle_hr_table_selection)

    def handle_employee_table_selection(self):
        # If a row is selected in employee table, clear HR table selection
        if self.admin_ui.employee_list_tbl.selectedIndexes():
            self.admin_ui.hr_list_tbl.blockSignals(True)
            self.admin_ui.hr_list_tbl.clearSelection()
            self.admin_ui.hr_list_tbl.blockSignals(False)

    def handle_hr_table_selection(self):
        # If a row is selected in HR table, clear employee table selection
        if self.admin_ui.hr_list_tbl.selectedIndexes():
            self.admin_ui.employee_list_tbl.blockSignals(True)
            self.admin_ui.employee_list_tbl.clearSelection()
            self.admin_ui.employee_list_tbl.blockSignals(False)
    ...
    def validate_edited_employee_data(self):
        ...
        if self.is_profile_picture_duplicate(profile_picture, current_employee_id=None):
            self.show_error("Validation Error", "This face is already used in another employee's profile picture. Please use a different photo.")
            return False, "Duplicate face detected in profile picture"
    ...
    def is_profile_picture_duplicate(self, profile_picture_path, current_employee_id=None):
        # Load ArcFace model
        face_app = insightface.app.FaceAnalysis(name='buffalo_sc', providers=['CPUExecutionProvider'])
        face_app.prepare(ctx_id=0, det_size=(640, 640))

        # Read and process the new profile picture
        img = cv2.imread(profile_picture_path)
        if img is None:
        ...
        new_embedding = faces[0].embedding.reshape(1, -1)
        profile_pictures_dir = os.path.join("resources", "profile_pictures")

        for fname in os.listdir(profile_pictures_dir):
            # Optionally skip current employee's own picture during edit
            if current_employee_id and fname.startswith(current_employee_id):
                continue
            existing_path = os.path.join(profile_pictures_dir, fname)
            existing_img = cv2.imread(existing_path)
            if existing_img is None:
                continue
            existing_faces = face_app.get(existing_img)
            if not existing_faces:
                continue
            existing_embedding = existing_faces[0].embedding.reshape(1, -1)
            score = cosine_similarity(new_embedding, existing_embedding)[0][0]
            if score > 0.65:  # Threshold for duplicate
                print(f"Duplicate face detected in profile picture: {fname}, score={score:.3f}")
                return True
        return False
    ...
    def handle_delete_employee(self):
        ...
        confirm_msg.setInformativeText(
            f"Are you sure you want to delete {employee_type} {employee_data['first_name']} {employee_data['last_name']}?\n\nThis action cannot be undone."
        )
        confirm_msg.setWindowTitle("Delete Confirmation")
        confirm_msg.setWindowFlags(Qt.Dialog | Qt.WindowTitleHint | Qt.CustomizeWindowHint | Qt.WindowStaysOnTopHint)
        ...
        if confirm_msg.exec() == QMessageBox.Yes:
            try:
                ...
                self.db.execute_query("DELETE FROM attendance_logs WHERE employee_id = ?", (employee_data['employee_id'],))
                # Delete feedback
                self.db.execute_query("DELETE FROM feedback WHERE created_by = ?", (employee_data['employee_id'],))
                # Delete fingerprint data and files
                cursor = self.db.execute_query("SELECT template_path FROM fingerprints WHERE employee_id = ?", (employee_data['employee_id'],))
                if cursor:
                    for row in cursor.fetchall():
                        fp_path = row[0]
                        if fp_path and os.path.exists(fp_path):
                            os.remove(fp_path)
                self.db.execute_query("DELETE FROM fingerprints WHERE employee_id = ?", (employee_data['employee_id'],))
                # Delete face model data and files
                cursor = self.db.execute_query("SELECT template_path FROM face_models WHERE employee_id = ?", (employee_data['employee_id'],))
                if cursor:
                    for row in cursor.fetchall():
                        face_paths = row[0].split(",") if row[0] else []
                        for face_path in face_paths:
                            face_path = face_path.strip()
                            if face_path and os.path.exists(face_path):
                                os.remove(face_path)
                self.db.execute_query("DELETE FROM face_models WHERE employee_id = ?", (employee_data['employee_id'],))
                # Delete profile picture
                if employee_data['profile_picture'] and os.path.exists(employee_data['profile_picture']):
                    os.remove(employee_data['profile_picture'])
                # Delete from Employee table
                self.db.execute_query("DELETE FROM Employee WHERE employee_id = ?", (employee_data['employee_id'],))
                ...
                QMessageBox.critical(None, "Delete Error", "An error occurred while deleting the employee record.")
            except Exception as e:
                print(f"Error during employee deletion: {e}")
                QMessageBox.critical(None, "Delete Error", "An unexpected error occurred during deletion.")
    ...
    def finalize_employee_enrollment(self):
        if hasattr(self, 'current_employee_data'):
            ...
            self.admin_ui.webcam_enroll_frame_lbl.setStyleSheet("")
            self.admin_ui.webcam_enroll_frame_lbl.setText("")
            self.admin_ui.webcam_enrollment_note_lbl.setText("")
            
            # Clear fingerprint success styling and text  
            self.admin_ui.fp_image_lbl.setStyleSheet("")
            self.admin_ui.fp_image_lbl.setText("")
            self.admin_ui.fp_enrollment_note_lbl.setText("")
    ...
    def handle_backup(self):
        try:
            ...
            self.show_success("Backup Configuration", "Backup configuration saved successfully.")
            ...
            self.show_success("Backup", f"Backup created successfully: {backup_file}")
    ...
    def scan_available_devices(self):
        ...
        try:
            ...
            if not fingerprint_available:
                QTimer.singleShot(1000, lambda: self.show_error("Device Not Available", "Fingerprint device not detected. Fingerprint authentication disabled."))
            if not camera_available:
                QTimer.singleShot(1000, lambda: self.show_error("Device Not Available", "Camera device not detected. Face ID authentication disabled."))
...
if __name__ == "__main__":
    if not os.path.exists("eals_database.db"):
        move_ui_and_resources_from_internal()
    app = QApplication(sys.argv)
    controller = show_splash_and_initialize()
    sys.exit(app.exec())
